...
function Get-NativeStandardOutput {
    ...
    $match=[regex]::Match($Text,'(?s)===== STANDARD OUTPUT =====\s*(.*?)\s*===== STANDARD ERROR =====')
...
function Test-DcdiagText {
    ...
    $output=Get-NativeStandardOutput $Text
    $failed=@($output -split "`r?`n" | ForEach-Object {$_.Trim()} | Where-Object {$_ -match '(?i)^\.{3,}\s+.+?\s+failed test\s+\S+'} | Sort-Object -Unique)
    if(!$failed.Count){return ,(New-Finding $Scope 'DCDiag' 'Green' 'DCDiag passed' 'No explicit failed-test result was found in the captured DCDiag output.' $Source)}
    $findings=@()
    foreach($line in $failed){
        $test=if($line -match '(?i)failed test\s+(\S+)'){$Matches[1]}else{'Unknown'}
        # Event-based tests can fail because of historical events even when current AD operations are healthy.
        $state=if($test -match '^(?i:DFSREvent|KccEvent|SystemLog|FrsEvent)$'){'Yellow'}else{'Red'}
        $title=if($state -eq 'Yellow'){"DCDiag event-history test requires review: $test"}else{"DCDiag test failed: $test"}
        $findings+=New-Finding $Scope 'DCDiag' $state $title $line $Source
    }
    return $findings
}
function Test-RepadminText {
    param([string]$ShowRepl,[string]$Summary,[string]$Scope)
    $showOutput=Get-NativeStandardOutput $ShowRepl
    $summaryOutput=Get-NativeStandardOutput $Summary
    if([string]::IsNullOrWhiteSpace($showOutput) -and [string]::IsNullOrWhiteSpace($summaryOutput)){
        return ,(New-Finding $Scope 'Replication' 'Yellow' 'Replication evidence missing' 'Repadmin output was not available.' 'repadmin')
    }

    $relationships=@();$parseWarnings=@()
    $csvLines=@($showOutput -split "`r?`n" | ForEach-Object {$_.Trim()} | Where-Object {$_ -match '^showrepl_(?:COLUMNS|INFO),'})
    $header=$csvLines | Where-Object {$_ -match '^showrepl_COLUMNS,'} | Select-Object -First 1
    $data=@($csvLines | Where-Object {$_ -match '^showrepl_INFO,'})
    if($header -and $data.Count){
        try{
            $headerColumns=@(($header | ConvertFrom-Csv -Header (0..20) | Select-Object -First 1).PSObject.Properties.Value)
            # ConvertFrom-Csv can parse the native Repadmin CSV directly after replacing the row marker header.
            $columnNames=@('RowType','Destination DSA Site','Destination DSA','Naming Context','Source DSA Site','Source DSA','Transport Type','Number of Failures','Last Failure Time','Last Success Time','Last Failure Status')
            $relationships=@(($data -join "`r`n") | ConvertFrom-Csv -Header $columnNames)
        }catch{$parseWarnings+=$_.Exception.Message}
    }

    $failedRelationships=@($relationships | Where-Object {
        $failures=0;$status=0
        [int]::TryParse([string]$_.'Number of Failures',[ref]$failures)|Out-Null
        [int]::TryParse([string]$_.'Last Failure Status',[ref]$status)|Out-Null
        $failures -gt 0 -or $status -ne 0
    })
    $summaryFailures=@($summaryOutput -split "`r?`n" | ForEach-Object {$_.Trim()} | Where-Object {
        $_ -match '^\S+\s+\S+\s+(\d+)\s*/\s*(\d+)\s+\d+' -and [int]$Matches[1] -gt 0
    })

    $findings=@()
    foreach($row in $failedRelationships){
        $detail="Destination=$($row.'Destination DSA'); Source=$($row.'Source DSA'); NamingContext=$($row.'Naming Context'); Failures=$($row.'Number of Failures'); LastStatus=$($row.'Last Failure Status'); LastSuccess=$($row.'Last Success Time')"
        $findings+=New-Finding $Scope 'Replication' 'Red' 'Replication relationship failure' $detail 'repadmin-showrepl-all.csv.txt'
    }
    foreach($line in $summaryFailures){$findings+=New-Finding $Scope 'Replication' 'Red' 'Replication summary failure' $line 'repadmin-replsummary.txt'}
    if(!$findings.Count){
        if($relationships.Count){
            $findings+=New-Finding $Scope 'Replication' 'Green' 'Replication healthy' ("{0} replication relationship(s) evaluated; all report zero failures and status 0." -f $relationships.Count) 'repadmin-showrepl-all.csv.txt'
        }elseif($summaryOutput -match '(?m)^\s*\S+\s+\S+\s+0\s*/\s*\d+\s+0'){
            $findings+=New-Finding $Scope 'Replication' 'Green' 'Replication healthy' 'Repadmin replication summary reports zero failures.' 'repadmin-replsummary.txt'
        }else{
            $findings+=New-Finding $Scope 'Replication' 'Yellow' 'Replication output could not be fully parsed' 'Review the raw Repadmin evidence. No confirmed nonzero failure row was identified.' 'repadmin'
        }
    }
    return $findings
}
...
try {
    ...
    foreach($analysisPackage in $packages){
        ...
        $dcFindings=@();$dcFindings+=Test-DcdiagText $dcdiagLocal 'dcdiag-local-verbose.txt' $scope;$dcFindings+=Test-DcdiagText $dcdiagEnterprise 'dcdiag-enterprise-comprehensive-verbose.txt' $scope;$dcFindings+=Test-RepadminText $showRepl $replSummary $scope;foreach($dnsFile in $dnsFiles){$dnsText=Get-Content -LiteralPath $dnsFile.FullName -Raw;$dnsScope=($dnsFile.BaseName -replace '^dcdiag-dns-','');$dcFindings+=Test-DnsText $dnsText $dnsScope $dnsFile.Name};if(!$dnsFiles.Count){$dcFindings+=Test-DnsText '' $scope 'dcdiag-dns'};$dcFindings+=@($sysvol.Findings);$dcFindings+=Get-EventFindings $analysisPackage.Events $scope
    ...
    $missing=@($allDomainControllers|Where-Object{$dcHostName=$_.HostName;@($packages|Where-Object{$_.Manifest.ComputerFqdn -ieq $dcHostName}).Count -eq 0});if($missing.Count){$body+=New-HtmlSection 'Collection Coverage Warning' ("<div class='notice'>No local collector package was provided for: $(ConvertTo-HtmlEncoded (($missing.HostName)-join ', ')). This is informational and does not reduce the health grade. Domain-wide Repadmin, enterprise DCDiag, DNS, reachability, and directory inventory evidence collected from the selected domain controller is used to assess the domain. Local event-log and local-configuration detail for uncollected domain controllers may be limited.</div>")}
