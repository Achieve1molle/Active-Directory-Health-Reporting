...
function Test-DcdiagText {
    ...
    $findings=@();if([string]::IsNullOrWhiteSpace($Text)){return ,(New-Finding $Scope 'DCDiag' 'Yellow' 'DCDiag evidence missing' 'The expected raw DCDiag file was not available.' $Source)}
    $failureLines=@($Text -split "`r?`n"|Where-Object{$_ -match '(?i)\bfailed test\b|\bfatal\b|\berror\b' -and $_ -notmatch '(?i)0 errors|no errors|error reporting'})
    if($failureLines.Count){foreach($line in $failureLines|Select-Object -First 50){$findings+=New-Finding $Scope 'DCDiag' 'Red' 'DCDiag failure' $line.Trim() $Source}}
    else{$findings+=New-Finding $Scope 'DCDiag' 'Green' 'DCDiag passed' 'No failed-test signature was found in the captured DCDiag output.' $Source}
    return $findings
}
function Test-RepadminText {
    param([string]$ShowRepl,[string]$Summary,[string]$Scope)
    $findings=@();$combined="$ShowRepl`n$Summary"
    ...
    $bad=@($combined -split "`r?`n"|Where-Object{$_ -match '(?i)\bfailed\b|last failure|result\s+[-:]\s*[1-9]|error\s+[1-9]' -and $_ -notmatch '(?i)0 fails|0 failures|no failures'})
    if($bad.Count){foreach($line in $bad|Select-Object -First 50){$findings+=New-Finding $Scope 'Replication' 'Red' 'Replication failure detected' $line.Trim() 'repadmin'}}
    else{$findings+=New-Finding $Scope 'Replication' 'Green' 'Replication healthy' 'No replication failure signature was found in the captured Repadmin output.' 'repadmin'}
    return $findings
}
...
try {
    ...
    # Domain-wide DNS server assessment. Local ZIP inventory proves role/service state for
    # collected DCs. A per-DC dcdiag DNS evidence file proves that the collector assessed
    # the named DC. When every discovered DC has DNS diagnostic evidence, count all assessed
    # DCs rather than counting only locally inventoried ZIP packages.
    ...
    $dnsLocalConfirmed=@{}
    foreach($p in $dnsPackageRows){
        $fqdn=[string]$p.Manifest.ComputerFqdn;if($fqdn){$dnsLocalConfirmed[$fqdn.ToLowerInvariant()]=$true}
    }
    $dnsDiagnosticTargets=@{}
    foreach($p in $packages){
        foreach($f in @(Get-ChildItem -LiteralPath $p.Path -Filter 'dcdiag-dns-*.txt' -File -ErrorAction SilentlyContinue)){
            $target=($f.BaseName -replace '^dcdiag-dns-','')
            if($target){$dnsDiagnosticTargets[$target.ToLowerInvariant()]=$true}
        }
    }
    $dnsConnectivityTargets=@{}
    foreach($p in $packages){
        foreach($r in @($p.Connectivity|Where-Object{[int]$_.Port -eq 53 -and $_.Open -eq $true})){
            $target=[string]$r.Destination;if($target){$dnsConnectivityTargets[$target.ToLowerInvariant()]=$true}
        }
    }
    $dnsRows=@()
    foreach($dc in $allDomainControllers){
        $host=[string]$dc.HostName;$key=$host.ToLowerInvariant();$short=($host -split '\.')[0].ToLowerInvariant()
        $localConfirmed=$dnsLocalConfirmed.ContainsKey($key) -or $dnsLocalConfirmed.ContainsKey($short)
        $diagnosticAssessed=$dnsDiagnosticTargets.ContainsKey($key) -or $dnsDiagnosticTargets.ContainsKey($short)
        $port53=$dnsConnectivityTargets.ContainsKey($key) -or $dnsConnectivityTargets.ContainsKey($short)
        $present=$localConfirmed -or $diagnosticAssessed -or $port53
        $basis=@();if($localConfirmed){$basis+='Local DNS role/service inventory'};if($diagnosticAssessed){$basis+='Per-DC DCDiag DNS evidence'};if($port53){$basis+='TCP 53 reachable'}
        $dnsRows+=[pscustomobject]@{HostName=$host;IPv4Address=$dc.IPv4Address;Site=$dc.Site;DnsServerPresent=$present;AssessmentBasis=if($basis.Count){$basis -join '; '}else{'No DNS server evidence collected'}}
    }
    $dnsServerCount=@($dnsRows|Where-Object DnsServerPresent -eq $true).Count
